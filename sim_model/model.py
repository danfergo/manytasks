...
class SimulationModel:
    ...
    def _spec_diff(self, lm_data, v, n, s):
        ...
        if self.rectify_fields:
            print('lm', lm.shape, self.s_ref_n.shape)
    ...
    def generate(self, depth):
        ...
        if self.apply_elastic_deformation:
            # cv2.imshow('depth map', to_normed_rgb(cv2.filter2D(self.bkg_depth - depth, -1, gkern2(55, 5))))
            # cv2.waitKey(-1)
            # print(depth.shape,
            #       (self.bkg_depth + cv2.filter2D(self.bkg_depth - depth, -1, gkern2(90, 35))).shape
            #       )
            depth = np.minimum(
                depth,
                self.bkg_depth -
                cv2.filter2D(self.bkg_depth - depth, -1, gkern2(55, 5))
            )
        ...
        # # exit()
        #
        # cam_matrix_tensor = o3d.core.Tensor(self.cam_matrix.intrinsic_matrix).cuda()
        # o3d_depth = o3d.t.geometry.Image(depth)
        #
        # o3d_cloud = o3d.t.geometry.PointCloud.create_from_depth_image(o3d_depth, cam_matrix_tensor)
        #
        # o3d_cloud = o3d_cloud + o3d_cloud
        # # print(dir(o3d_cloud))
        # # device = o3d.core.Device("CPU:0")
        # # o3d_cloud_cpu = o3d_cloud.cpu()
        #
        # depth_ = o3d_cloud.project_to_depth_image(640, 480, cam_matrix_tensor)
        #
        #
        # depth_ = np.array(depth_).squeeze()

        # cv2.imshow('xx', to_normed_rgb(depth_))
        #
        # cv2.waitKey(-1)
        #
        # print('depth', )
